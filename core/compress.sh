...
compress()
{
    ...
    local cnt=1 size_src=0 size_dst=0 srcRes
    echo $dirVec
    for src in $dirVec/*.yuv; do
        [[ ! -f $src ]] && continue
        detect_resolution_string $src; srcRes=$REPLY
        dst=${src%.*}.nut
        printf "\\x1B[2K\r[%d/%d] %s %s -> %s\r" $cnt $cnt_total $srcRes ${src##*/} ${dst##*/}
        local stderr
        if ! stderr=$(ffmpeg -y -loglevel error -s $srcRes -i $src -vcodec ffv1 -level 3 -f nut -threads 4 -coder 1 -context 1 -g 1 -slices 4 $tmpfile 2>&1 >/dev/null); then
            printf "\n%s\n" "$stderr"
        fi
        mv $tmpfile $dst
