...
entrypoint()
{
	...
	# Scheduling
	#
	progress_begin "[1/5] Scheduling..." "$PRMS" "$VECTORS" "$CODEC_CNT"
    ...
    for CODEC_LONG; do
        ...
        if [[ -z $hide_banner ]]; then
            ...
            [[ -n "$EXTRA" ]] && info="$info {$EXTRA}"

            echo "$timestamp $info" | tee -a $report $report_kw $report_bdrate >/dev/null
        ...
        if [[ -n $bdrate ]]; then
            ...
            local tag="$exeHash[--preset $codec_preset --threads $codec_threads]${codec_prms:+[$codec_prms]}{$codec_args}"
